...
save_error_report_if_enabled() {
    ...
    mkdir -p ~/.btptfexport
    echo "$timestamp|$event_name|$reason|$version$additional_items_serialized" >> ~/.btptfexport/installer-error.log

    say_error "An error was encountered during install: $reason"
    say_error ""
    say_error "You can file an issue at https://github.com/SAP/terraform-exporter-btp/issues/new?assignees=&labels=bug%2Cneeds-triage&projects=&template=bug_report.yml&title=%5BBUG%5D"
}

say_error() {
    printf "install-btptfexport: ERROR: %b\n" "$1" >&2
}

say() {
    printf "install-btptfexport: %b\n" "$1"
}
...
DEFAULT_BASE_URL="https://btptfexportrelease.azureedge.net/btptfexport/standalone/release"
...
install_folder="/opt/microsoft/btptfexport"
verbose=false
...
do
  ...
  case "$name" in
    ...
    -h|-?|--help)
        ...
        echo "Terraform Exporter for SAP BTP CLI Installer"
        echo "Usage: $script_name [--version <VERSION>] [--install-location <LOCATION>]"
        echo "       $script_name -h|-?|--help"
        echo ""
        echo "$script_name is a simple command for downloading and installing the btptfexport CLI"
        echo ""
        echo "Options:"
        ...
        echo "                                download URL."
        echo ""
        echo "  -i,--install-folder <FOLDER>  Install btptfexport CLI to FOLDER. Default is"
        echo "                                /usr/local/bin"
        echo ""
...
if [ -z "$version" ]; then
    url="$base_url/btptfexport-$platform-$architecture.$extension"
else
    url="$base_url/$version/btptfexport-$platform-$architecture.$extension"
fi
...
tmp_folder="$(mktemp -d)";
compressed_file_path="$tmp_folder/btptfexport-$platform-$architecture.$extension"
...
bin_name="btptfexport-$platform-$architecture"
...
if [ "$symlink_folder" != "" ]; then
    ...
    if ! $ln_preface ln -fs "$install_folder/$bin_name" "$symlink_folder/btptfexport"; then
        say_error "Could not create symlink to btptfexport in $symlink_folder"
...
if [ "$symlink_folder" != "" ]; then
    say "Symlink created at $symlink_folder/btptfexport and pointing to $install_folder/$bin_name"
fi
